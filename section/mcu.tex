...
\section{Preemptive skedulering}
FreeRTOS benytter en prioritetsbaseret preemptive skeduleringsalgoritme.
Når et operativ system opererer med en preemptive skeduleringsalgoritme kan kørende tasks \textit{preemptes}, hvilket betyder tasken bliver stoppet og skiftet ud med en anden task af højere prioritet.\newline
...
FreeRTOS skeduleringsalgoritme sørger så for at den task med højest prioritet bliver givet processortid ligeså snart den bliver placeret i \textit{ready queuen}.
...
\section{Interrupt eksekvering og task skedulering}
...
ISRen prioriteres højest, og den lavt prioriteret task pauses indtil ISR'en er færdig.
...
For at sikre, at microcontrolleren sætter alle andre opgaver på pause, og den begynder at sample på det korrekte tidspunkt, implementeres samplingen i en ISR.
...
Microcontrolleren har to identiske ADC moduler, som opererer uafhængigt af hinanden. 
...
Det betyder at data fra samplingen bliver gemt i sequence 3 registeret indtil næste gang interruptet kaldes. 
...
Da sample handleren kører periodisk i form af en ISR, har den højest prioritet til processortid, derfor skal alle andre processoropgaver foregå i den mellemliggende tid.
...
Functionspointeren benyttes til at kalde lydeffektmodulerne, og den booleanske variabel benyttes til at aktivere/deaktivere lydeffekterne.\newline
...
For at tilføje et effektmodul, tilføjes et nyt element til $\mathtt{mcb\_pool}$, \textit{function pointerent} sættes til at \textit{pointe} til effektroutinen og den boolske variabel sættes til $\mathtt{TRUE}$.
