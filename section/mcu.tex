...
FreeRTOS scheduleringsalgoritme sørger så for at den task med højest prioritet bliver givet processortid ligeså snart den bliver placeret i \textit{ready queuen}.
...
Denne ISR sørger for at hente lyddata fra ADC'en og finder frem til hvilke moduler der skal medtages i databehandlingen samt række følgen disse tilføres signalet. %og hvordan og til hvilke buffere, \texttt{Buffer} eller \texttt{Rev. Buffer} data skal gemmes i.
...
For at sikre, at microcontrolleren sætter alle andre opgaver på pause, og begynder at sample på det korrekte tidspunkt, implementeres samplingen i en ISR.
...
Da sample handleren kører periodisk i form af en ISR, har den højst prioritet til processortid, derfor skal alle andre processoropgaver foregå i den mellemliggende tid.
