...
Bufferen der blev valgt at implementere dette er af en cyklisk natur, hvor der til hver samplingstid udlæses en værdi fra den af pointeren indikerede plads, pladsen nulstilles så og pointeren forøges, så der til næste sample udlæses fra den næste plads i bufferen.
...
\textbf{To:} Afstanden mellem en given overflade, lydkilden og modtageren kan variere, dette giver refleksioner med forskellig forsinkelse samt forskellige amplituder da lydbølgen aftager i styrke jo længere den bevæger sig.\newline 
...
\husk{Overståen linje er meget meget mærkelig}
...
Filteret og dets koefficienter bliver ved run-time beregnet efter brugeres angivelse af cutoff frekvensen. Filteret designes og koefficienterne bliver beregnet via \textit{frequency sampling} metoden.

Grunden til \textit{frequency sampling} valges som designmetode er, at algoritmen er baseret på invers diskret frekvens/tid fourier transformation (IDFT), hvilket kan beregnes effektivt via FFT.\newline
...
Det blev valgt at implementeret det digitale filter som et FIR filter, fordi FIR filtre giver mulighed for at have lineær negativ fase.\newline
...
Det er derfor vigtigt at optimere og begrænse alle beregninger så de bruger den mindste tid mulig. 
...
Da det endte med en pladsmangel på mikrokontrolleren, når der skal ligge flere forskellige moduler på.
